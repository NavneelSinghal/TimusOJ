...
void solve(int) {
    ...
    vector<int> length_ending_at(n + 1, 0);
    d[0] = inf;

    int len = 0;
    
    // need to process in increasing order of x, and y in case of ties
    for (int i = 0; i < n; ++i) {
        
        int y = a[i].r, l = 0, r = len, ans = len - 1;
       
        // find rightmost index ans such that d[ans] > y, i.e., sth like upper_bound
        while (l <= r) {
            int m = l + (r - l) / 2;
            if (d[m] > y) {
                l = m + 1;
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (length_ending_at[i] == len && a[i].r > last_taken) {
            cout << a[i].i + 1 << ' ';
            --len;
        }
    }
